...
<body>
    ...
    <div class="main-content">
        <div class="container">
            <a href="{{ route('transaksi.index') }}" class="btn btn-warning btn-sm">Kembali</a>
            <a href="{{ route('detail.cetak', ['no_nota' => $transaksi->no_nota]) }}" class="btn btn-warning btn-sm">Cetak</a>
        
            <h1>Detail Transaksi: {{ $transaksi->no_nota }}</h1>
<table class="table-detail">
    <tbody>
        ...
        <a href="{{ route('detail.barang.add', ['no_nota' => $transaksi->no_nota]) }}" class="btn btn-primary mb-3">Tambah Barang</a>
        <thead>
            <tr>
                <th>Nama Barang</th>
                <th>Jumlah</th>
        ...
        <tbody>
            @foreach($transaksi->detailBarangs as $detail)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('detail.barang.edit', $detail->id) }}" class="btn btn-warning btn-sm">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <form action="{{ route('detail.barang.delete', $detail->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
    ...
    <table class="table table-striped">
        <a href="{{ route('detail.service.add', $transaksi->no_nota) }}" class="btn btn-primary mb-3">Tambah Service</a>
        <thead>
            <tr>
                <th>Nama Service</th>
                <th>Jumlah</th>
        ...
        <tbody>
            @foreach($transaksi->detailServices as $detail)
                <tr>
                    ...
                    <td>Rp {{ number_format($detail->subtotal, 2) }}</td>
                    <td>
                        <a href="{{ route('detail.service.edit', $detail->id) }}" class="btn btn-warning btn-sm">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <form action="{{ route('detail.service.delete', $detail->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus?')">
                                <i class="bi bi-trash"></i>
