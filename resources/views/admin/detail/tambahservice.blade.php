...
<body>
    ...
    <div class="container">
        <h1>Tambah Detail Service</h1>
    
        <form action="{{ route('detail.service.save', ['no_nota' => $transaksi->no_nota]) }}" method="POST">
            @csrf
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="service_id" name="service_id" required>
                    ...
                    @foreach($services as $service)
                        <option value="{{ $service->id }}" data-harga="{{ $service->harga }}">{{ $service->nama_jasa }} - Rp {{ number_format($service->harga, 2) }}</option>
                    @endforeach
                </select>
            </div>
    ...
    <script>
        document.getElementById('service_id').addEventListener('change', function() {
            ...
            const harga = selectedService.getAttribute('data-harga');
