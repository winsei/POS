...
<body>
    ...
    <form action="{{ route('transaksi.save') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" no_nota="customer" name="customer" value="{{ old('customer') }}" required>
        </div>

        <div class="mb-3">
            <label for="tanggal" class="form-label">Tanggal</label>
            <input type="date" class="form-control" no_nota="tanggal" name="tanggal" value="{{ old('tanggal') }}" required>
        </div>

        <div class="mb-3">
            <label for="no_polisi" class="form-label">No Polisi</label>
            <input type="text" class="form-control" no_nota="no_polisi" name="no_polisi" value="{{ old('no_polisi') }}" required>
        </div>

        <!-- Harga -->
        <div class="mb-3">
            <label for="total" class="form-label">Total</label>
            <input type="number" step="0.01" class="form-control" no_nota="total" name="total" value="{{ old('total') }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="jenis_pembayaran" class="form-select">
                ...
                <option value="cash" {{ (isset($transaksi) && $transaksi->jenis_pembayaran == 'cash') ? 'selected' : '' }}>Cash</option>
                <option value="debit" {{ (isset($transaksi) && $transaksi->jenis_pembayaran == 'debit') ? 'selected' : '' }}>Debit</option>
            </select>
        </div>

        <!-- Tombol Simpan -->
        <button type="submit" class="btn btn-success">Tambah Keuangan</button>
